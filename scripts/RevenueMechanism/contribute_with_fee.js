...
async function main() {
    ...
    const BSKT_ADDRESS = "0x5506fc40eD13DaD2fC35e0eAfE37a96b313a023f";
    ...
    const lpToken = await ethers.getContractAt("BasketTokenStandardPair", bsktPair, signer);
    ...
    const factory = await ethers.getContractAt("Factory", await bskt.factory(), signer);
    ...
    try {
        ...
        const tx = await bskt.contribute(50, {
        ...
        const contributedEvent = receipt.logs
            .filter(log => {
                try {
                    ...
                    return parsedLog && parsedLog.name === "ContributedToBSKT";
                } catch (e) {
                    return false;
                }
            })
            .map(log => bskt.interface.parseLog(log));
        ...
        if (contributedEvent.length > 0) {
            console.log("\n   ContributedToBSKT Event Found:");
            console.log("   Basket:", contributedEvent[0].args.bskt);
