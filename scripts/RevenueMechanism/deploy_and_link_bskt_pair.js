...
const bsktPairBeacon = require("../../artifacts/contracts/BSKTBeacon.sol/BSKTBeacon.json");
...
async function main() {
  ...
  if (!IMPLEMENTATION_ADDRESS) {
    console.log("\nDeploying new BSKT Pair implementation...");
    const BSKTPair = await ethers.getContractFactory("BasketTokenStandardPair");
    const bsktPair = await BSKTPair.deploy();
    const deployTx = await bsktPair.deploymentTransaction();
    ...
    IMPLEMENTATION_ADDRESS = await bsktPair.getAddress();
    ...
    await verify(IMPLEMENTATION_ADDRESS, [], "contracts/BSKTPair.sol:BSKTPair");
  } else {
  ...
  const BEACON_ADDRESS = "0x03d53767140B4660d5aF580D0C283519D0F0741f"; // BSKTPair Beacon Address
  const bsktPairBeaconContract = new ethers.Contract(BEACON_ADDRESS, bsktPairBeacon.abi, signer);

  // 5. Check ownership
  const owner = await bsktPairBeaconContract.owner();
  console.log("\nBeacon owner:", owner);
  ...
  const currentImpl = await bsktPairBeaconContract.implementation();
  ...
  const updateTx = await bsktPairBeaconContract.upgradeTo(IMPLEMENTATION_ADDRESS, {
  ...
  const newImpl = await bsktPairBeaconContract.implementation();
