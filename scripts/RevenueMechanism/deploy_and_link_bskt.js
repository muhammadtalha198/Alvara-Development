...
const bsktBeacon = require("../../artifacts/contracts/BSKTBeacon.sol/BSKTBeacon.json");
...
async function main() {
  ...
  if (!IMPLEMENTATION_ADDRESS) {
    ...
    IMPLEMENTATION_ADDRESS = await bskt.getAddress();
    ...
    await verify(IMPLEMENTATION_ADDRESS, [], "contracts/BSKT.sol:BasketTokenStandard");
  } else {
  ...
  const bsktBeaconContract = new ethers.Contract(BEACON_ADDRESS, bsktBeacon.abi, signer);

  // 5. Check ownership
  const owner = await bsktBeaconContract.owner();
  console.log("\nBeacon owner:", owner);
  ...
  const currentImpl = await bsktBeaconContract.implementation();
  ...
  const updateTx = await bsktBeaconContract.upgradeTo(IMPLEMENTATION_ADDRESS, {
  ...
  const newImpl = await bsktBeaconContract.implementation();
