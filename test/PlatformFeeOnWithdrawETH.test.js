...
describe.only("Platform Fee Deduction On Withdraw ETH", () => {
  ...
  it("should revert when trying to withdraw 0 LP tokens", async function () {
    ...
    const userLpBalance = await bsktPair.balanceOf(user1.address);

    // Approve bskt to spend LP tokens (even though we're withdrawing 0)
    await bsktPair.connect(user1).approve(bsktAddress, userLpBalance);

    // Attempt to withdraw 0 LP tokens - should revert
    await expect(bskt.connect(user1).withdrawETH(0, 100, calculateDeadline(20))) // 100 = 1% buffer
  ...
  it("should revert when trying to withdraw 0 LP tokens", async function () {
    ...
    // Attempt to withdraw 0 LP tokens - should revert
    await expect(
      bskt.connect(user1).withdrawETH(userLpBalance, 100, calculateDeadline(0))
    ) // 100 = 1% buffer
  ...
  it("should successfully withdraw ETH with 0% platform fee", async function () {
    ...
    const userLpBalancePost = await bsktPair.balanceOf(user1.address);
