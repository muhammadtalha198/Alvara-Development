...
describe.only("Royalty", () => {
  ...
  describe("Royalty Functionality", function () {
    it("Should return 0 royalty for a sale price of 0", async function () {
      ...
      const { royaltyAmount } = await bsktInstance.royaltyInfo(0, 0);
    ...
    it("Should return default royalty receiver and calculate correct royalty amount", async function () {
      ...
      const { receiver, royaltyAmount } = await bsktInstance.royaltyInfo(
        0,
        ethValue
      );
    ...
    it("Should update royalty receiver on the factory and reflect the change in BSKT royalty info", async function () {
      ...
      const [receiver] = await bsktInstance.royaltyInfo(0, ethValue);
      ...
      const [updatedReceiver, _] = await bsktInstance.royaltyInfo(
        0,
        ethers.parseEther("1")
      );
    ...
    it("Should update royalty percentage on the factory and reflect updated royalty amount in BSKT", async function () {
  ...
  describe("BSKT Royalty & Whitelisting Functionality", function () {
    ...
    it("Should handle minimum non-zero sale price", async () => {
      ...
      const { royaltyAmount } = await bsktInstance.royaltyInfo(0, minPrice);
    ...
    it("Should calculate correct royalties before and after royalty percentage update", async () => {
      ...
      let { royaltyAmount } = await bsktInstance.royaltyInfo(0, salePrice);
      ...
      ({ royaltyAmount } = await bsktInstance.royaltyInfo(0, salePrice));
    ...
    it("Should reject approve to non-whitelisted contract", async () => {
      ...
      await expect(
        bsktInstance
          .connect(seller)
          .setApprovalForAll(await marketplace.getAddress(), true)
      ).to.be.revertedWithCustomError(bsktInstance, "ContractNotWhitelisted");
    });
    it("Should allow transfer to whitelisted marketplace", async () => {
      ...
      await marketplace
        .connect(seller)
        .listNFT(await bsktInstance.getAddress(), 0, price);
      await marketplace
        .connect(buyer)
        .buyNFT(await bsktInstance.getAddress(), 0, { value: price });

      const newOwner = await bsktInstance.ownerOf(0);
  ...
  describe("Coverage Edge Cases", () => {
    it("Should transfer NFT to a non-contract wallet", async () => {
      ...
      expect(await bsktInstance.ownerOf(0)).to.equal(user1.address);
      expect(await bsktInstance.balanceOf(user1.address)).to.equal(1);
      expect(await bsktInstance.balanceOf(seller.address)).to.equal(0);
    });
    it("Should support ERC2981 interface", async () => {
      ...
      expect(
        await bsktInstance.supportsInterface(ERC2981_INTERFACE_ID)
      ).to.equal(true);
    });
    ...
    it("Should allow approve for whitelisted address", async () => {
      ...
      await factory.addWhitelistedContract(await marketplace.getAddress());
      await expect(bsktInstance.connect(seller).approve(await marketplace.getAddress(), 0)).to.not
        .be.reverted;
    });
    ...
    it("Should revert approve for non-whitelisted address", async () => {
      ...
      await expect(
        bsktInstance.connect(seller).approve(nonWhitelisted.getAddress(), 0)
      ).to.be.revertedWithCustomError(bsktInstance, "ContractNotWhitelisted");
    });
  });
});
