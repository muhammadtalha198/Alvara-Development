...
describe("PlatformFee", () => {
  ...
  before(async function () {
    ...
    try {
      ...
      console.log("Could not get bskt owner address. Error:", error.message);
      return;
    }
  ...
  describe("Platform Fee Configuration", function () {
    it("01- should retrieve the fee configuration", async function () {
      ...
      expect(feeConfig.bsktCreationFee).to.be.gte(0, "bskt creation fee should be >= 0");
      expect(feeConfig.bsktCreationFee).to.be.lte(MAX_FEE, "bskt creation fee should be <= 100%");
    ...
    it("02- should update the fee config", async function () {
      ...
      const newbsktCreationFee = 30; // 0.3%
      ...
      logHelper.log("02-setPlatformFeeConfig-UpdatePlatformFee", {
        tx: tx.hash,
        bsktCreationFee: newbsktCreationFee,
      ...
      expect(updatedFeeConfig.bsktCreationFee).to.equal(newbsktCreationFee, "bskt creation fee not updated correctly");
    ...
    it("03- should fail when setting fee greater than 0.5%", async function () {
      ...
      // Attempt to update with invalid bskt creation fee
      await expect(
      ...
      await expect(
        factory.setPlatformFeeConfig(
          initialFeeConfig.bsktCreationFee,
          invalidFee,
          initialFeeConfig.withdrawalFee
        )
      ).to.be.reverted;
    ...
    it("04- should allow setting fees to 0%", async function () {
      ...
      logHelper.log("04-setPlatformFeeConfig-SetZeroFees", {
        tx: tx.hash,
        bsktCreationFee: zeroFee.toString(),
      ...
      expect(updatedFeeConfig.bsktCreationFee).to.equal(zeroFee, "bskt creation fee not updated to zero");
