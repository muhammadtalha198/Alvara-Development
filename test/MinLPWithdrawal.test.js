...
describe.only("LP Withdrawal Range", function () {
  ...
  let user2, bskt, bsktPair, wETH, alva, mtToken, router;
  let bsktAddress,
    bsktPairAddress,
    factoryAddress,
    wETHAddress,
  ...
  const description = "This is a test BSKT for withdrawal testing";
  const buffer = 100;
  const _id = "test-withdrawal-bskt";
  ...
  beforeEach(async function () {
    ...
    await bskt.connect(user1).contribute(buffer, calculateDeadline(20), {
  ...
  //   const userBalance = await bsktPair.balanceOf(selectedUser.address);
  //   await bsktPair
  //     .connect(selectedUser)
  //     .approve(bsktAddress, ethers.parseEther(WITHDRAWAL_LP_AMOUNT.toString()));
  ...
  //   // Create a new bskt with 9000 ETH using owner address
  //   const weights = ["5000", "5000"];
  //   const createBSKTTx = await factory.createBSKT(
  //     name,
  //     symbol,
  ...
  //   await newbsktPair
  //     .connect(selectedUser)
  //     .approve(newBsktAddress, WITHDRAWAL_LP_AMOUNT);
  ...
  //   await newBsktPair.approve(newBsktAddress, WITHDRAWAL_LP_AMOUNT);

  //   // Expect revert with custom error using the correct owner signer
  //   await expect(
  //     newBskt.withdraw(WITHDRAWAL_LP_AMOUNT, buffer, calculateDeadline(20))
  //   ).to.be.revertedWithCustomError(newBskt, "InvalidWithdrawalAmount");
  // });
