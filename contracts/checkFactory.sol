...
contract Factory is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IFactory {
    ...
    function updateBSKTPairImplementation(address _bsktPairImplementation) external onlyRole(UPGRADER_ROLE) {
        ...
        if (_bsktPairImplementation == address(0) || !AddressUpgradeable.isContract(_bsktPairImplementation)) 
            revert InvalidAddress();
    ...
    function updateCollectionURI(string calldata _collectionURI) external onlyRole(URI_MANAGER_ROLE) {
        ...
        if (bytes(_collectionURI).length == 0) 
            revert EmptyStringParameter("URI");
    ...
    function updateMinBSKTCreationAmount(uint256 _minBSKTCreationAmount) external onlyRole(ADMIN_ROLE) {
        ...
        if (_minBSKTCreationAmount == 0 || _minBSKTCreationAmount == minBSKTCreationAmount) 
    ...
    function addWhitelistedContract(address contractAddr) external onlyRole(WHITELIST_MANAGER_ROLE) {
        ...
        if (contractAddr == address(0) || whitelistedContracts[contractAddr] || !AddressUpgradeable.isContract(contractAddr)) 
            revert InvalidWhitelistAddress(contractAddr, whitelistedContracts[contractAddr]);
    ...
    function setPlatformFeeConfig(uint16 _bsktCreationFee, uint16 _contributionFee, uint16 _withdrawalFee) external onlyRole(FEE_MANAGER_ROLE) {
        ...
        if (_bsktCreationFee > DEFAULT_FEE || _contributionFee > DEFAULT_FEE || _withdrawalFee > DEFAULT_FEE) 
            revert InvalidFee();
    ...
    function setFeeCollector(address _feeCollector) external onlyRole(FEE_MANAGER_ROLE) {
        ...
        if (_feeCollector == address(0) || _feeCollector == platformFeeConfig.feeCollector) 
            revert InvalidAddress();
    ...
    function totalBSKT() external view returns (uint256) { 
    ...
    function setMinLpWithdrawal(uint256 newMin) external onlyRole(ADMIN_ROLE) { 
        _minLpWithdrawal = newMin; emit MinLpWithdrawalUpdated(newMin); 
    }
